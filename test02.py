...
def detect_from_camera_dual(model1, model2, class_names, device):
    ...
    while True:
        ...
        img = letterbox(frame_yolo, img_size, stride=stride)[0]
        ...
        for det in pred1:
            if len(det):
                det[:, :4] = scale_coords(img_tensor1.shape[2:], det[:, :4], frame.shape).round()
                for *xyxy, conf, cls in det:
                    label = f'{names[int(cls)]} {conf:.2f}'
                    cv2.rectangle(frame_yolo, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (0, 255, 0), 2)
                    cv2.putText(frame_yolo, label, (int(xyxy[0]), int(xyxy[1]) - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                                (255, 255, 255), 1)

        # ----- RCNN Inference -----
        frame_rcnn = frame.copy()
        image_pil = Image.fromarray(cv2.cvtColor(frame_rcnn, cv2.COLOR_BGR2RGB))
        ...
        frame_rcnn = cv2.cvtColor(np.array(image_pil), cv2.COLOR_RGB2BGR)

        # Show both outputs side-by-side
        combined = np.hstack((frame_yolo, frame_rcnn))
        cv2.imshow("YOLOv5 vs Faster R-CNN", combined)
