...
mot_tracker = DeepSort(
    model_path='./trained_models/checkpoints/ckpt.t7',
    max_dist=0.2,
    min_confidence=0.3,
    nms_max_overlap=1.0,
...
def detect_from_camera_dual(model1, model2, class_names, device):
    ...
    while True:
        ...
        if drone_detections_np.shape[0] > 0:
            ...
            xyxy = drone_detections_np[:, :4]
            confidences = drone_detections_np[:, 4]
            classes = np.array([1] * len(drone_detections_np))  # All drones
            ...
            # Update tracker
            tracked_objects = mot_tracker.update(bbox_xywh, confidences, classes, np.array(base_frame))
        else:
            tracked_objects = []
        ...
        for track in tracked_objects:
            ...
            x1, y1, x2, y2, cls_id, track_id = track
